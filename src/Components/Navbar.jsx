...
import {selectCategories  } from "../features/catagary/catagarySlice";
...
function Navbar() {
  ...
  const catagaryHandler = (e) =>{
    e.preventDefault();
    dispatch(selectCategories(inputValue));
    setInputValue(" ")
  }
  return (
   ...
   <div>
       ...
       <button className='rounded-full hover:bg-gray-500 py-2 px-3 cursor-pointer' onClick={() => dispatch(selectCategories('All'))}>All</button>
      <button className='rounded-full hover:bg-gray-500 py-2 px-3 cursor-pointer' onClick={() => dispatch(selectCategories('Photo'))}>Photo</button>
      <button className='rounded-full hover:bg-gray-500 py-2 px-3 cursor-pointer' onClick={() => dispatch(selectCategories('Video'))}>Video</button> 
    ...
    <form onSubmit={catagaryHandler} className="ml-5 flex w-[40%] items-center justify-between">
      <input
      ...
      {/* <!--Search icon--> */}
      <button
        type = 'submit' className="input-group-text flex items-center whitespace-nowrap rounded-full px-3 py-1.5 text-center text-base font-normal text-neutral-700 dark:text-neutral-200 cursor-pointer hover:bg-slate-400"
        id="basic-addon2">
        <svg
